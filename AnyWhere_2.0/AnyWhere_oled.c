...
static void aw_gtk_class_init(AwGtkClass *kclass)
{
    ...
    properties[PROPERTY_INTERFACE_SELECT] = g_param_spec_int("select", NULL, NULL, -2, G_MAXINT32, -1, G_PARAM_READWRITE);
    properties[PROPERTY_NETWORK_MODE] = g_param_spec_uint("network", NULL, NULL, 0, G_MAXUINT32, 0, G_PARAM_READWRITE);
    ...
    properties[PROPERTY_JPEG_DISK_SPACE] = g_param_spec_pointer("JpegDisk", NULL, NULL, G_PARAM_READWRITE);
    properties[PROPERTY_JPEG_RESOLUTION] = g_param_spec_pointer("JpegResolution", NULL, NULL, G_PARAM_READWRITE);
    properties[PROPERTY_JPEG_SHOOT] = g_param_spec_pointer("JpegShoot", NULL, NULL, G_PARAM_READWRITE);
    properties[PROPERTY_JPEG_FOMAT] = g_param_spec_pointer("JpegFormat", NULL, NULL, G_PARAM_READWRITE);
    properties[PROPERTY_MP4_WORKTIME] = g_param_spec_pointer("VideoTime", NULL, NULL, G_PARAM_READWRITE);
    ...
    properties[PROPERTY_LIVE_WORKTIME] = g_param_spec_pointer("LiveTime", NULL, NULL, G_PARAM_READWRITE);
    ...
    properties[PROPERTY_IMAGE_URL] = g_param_spec_pointer("ImageUrl", NULL, NULL, G_PARAM_READWRITE);
...
gint oled_start()
{
    ...
    g_signal_connect(entry, "notify::interface", G_CALLBACK(judge_interface), NULL);        //OK
    g_signal_connect(entry, "notify::jpeglist", G_CALLBACK(judge_interface_select), NULL);
    g_signal_connect(entry, "notify::videolist", G_CALLBACK(judge_interface_select), NULL);
    g_signal_connect(entry, "notify::livelist", G_CALLBACK(judge_interface_select), NULL);
    g_signal_connect(entry, "notify::systemlist", G_CALLBACK(judge_interface_select), NULL);
    g_signal_connect(entry, "notify::select", G_CALLBACK(judge_interface_select), NULL);
    g_signal_connect(entry, "notify::network", G_CALLBACK(judge_navigat), NULL);
    g_signal_connect(entry, "notify::IPaddr", G_CALLBACK(judge_navigat), NULL);             //OK
    g_signal_connect(entry, "notify::battery", G_CALLBACK(judge_navigat), NULL);            //OK
    g_signal_connect(entry, "notify::JpegDisk", G_CALLBACK(judge_jpeg_state), NULL);
    g_signal_connect(entry, "notify::JpegResolution", G_CALLBACK(judge_jpeg_state), NULL);
    g_signal_connect(entry, "notify::JpegShoot", G_CALLBACK(judge_jpeg_state), NULL);
    g_signal_connect(entry, "notify::JpegFormat", G_CALLBACK(judge_jpeg_state), NULL);
    g_signal_connect(entry, "notify::VideoTime", G_CALLBACK(judge_mp4_state), NULL);
    g_signal_connect(entry, "notify::VideoResolution", G_CALLBACK(judge_mp4_state), NULL);
    g_signal_connect(entry, "notify::VideoBitrate", G_CALLBACK(judge_mp4_state), NULL);
    g_signal_connect(entry, "notify::VideoFramerate", G_CALLBACK(judge_mp4_state), NULL);
    g_signal_connect(entry, "notify::LiveTime", G_CALLBACK(judge_live_state), NULL);
    g_signal_connect(entry, "notify::LiveResolution", G_CALLBACK(judge_live_state), NULL);
    g_signal_connect(entry, "notify::LiveBitrate", G_CALLBACK(judge_live_state), NULL);
    g_signal_connect(entry, "notify::LiveFramerate", G_CALLBACK(judge_live_state), NULL);
...
gint show_jpeg_state()
{
    ...
    g_object_get(G_OBJECT (entry), "IPaddr", &ip_addr, "JpegDisk", &disk_space, "JpegShoot", &shoot, NULL);
    cairo_t *cr1 = cairo_create(surface);     //网络工作模式，图片
    cairo_t *cr2 = cairo_create(surface);     //电量，图片
    cairo_t *cr3 = cairo_create(surface);     //工作模式，图片
    cairo_t *cr4 = cairo_create(surface);     //IP时地址，字符串
...
gint show_jpeg_setting()  //包含拍照模式，连拍间隔，采用Glist
{
    ...
    g_object_get(G_OBJECT (entry), "jpeglist", &list, "select", &select, NULL);
    if (select < -1 || select > 2)
    {
        g_print("jpeg list property has 3 items, select wrong, (-2 < select < 3)!\n");
        return -1;
    }
...
gint show_mp4_state()
{
    ...
    g_object_get(G_OBJECT (entry), "IPaddr", &ip_addr, "VideoTime", &worktime, NULL);
    cairo_t *cr1 = cairo_create(surface);     //网络工作模式，图片
    cairo_t *cr2 = cairo_create(surface);     //电量，图片
    cairo_t *cr3 = cairo_create(surface);     //工作模式，图片
    cairo_t *cr4 = cairo_create(surface);     //IP时地址，字符串
    ...
    cairo_set_font_size(cr5, 24.0);
    cairo_set_source_rgb(cr5, 1.0, 1.0, 1.0); 
    cairo_move_to(cr5, 28.0, 39.0);     //工作时间位置
    ...
    cairo_set_font_size(cr8, 14.0);
    cairo_set_source_rgb(cr8, 1.0, 1.0, 1.0); 
    cairo_move_to(cr8, 88.0, 60.0);     //码率位置
...
gint show_mp4_setting()
{
    ...
    gchar *item2 = NULL;
    gchar *item3 = NULL;
    g_object_get(G_OBJECT (entry), "videolist", &list, "select", &select, NULL);
    if (select < -1 || select > 4)
    {
        g_print("mp4 list property has 5 items, select wrong, (-2 < select < 5)!\n");
        return -1;
    }
    cairo_t *cr1 = cairo_create(surface);     //工作模式，图片
    cairo_t *cr2 = cairo_create(surface);     //三角箭头，图片
...
gint show_live_state()
{
    ...
    g_object_get(G_OBJECT (entry), "IPaddr", &ip_addr, "LiveTime", &worktime, "LiveResolution", &resolution,
    ...
    cairo_set_font_size(cr5, 24.0);
    cairo_set_source_rgb(cr5, 1.0, 1.0, 1.0); 
    cairo_move_to(cr5, 28.0, 39.0);     //工作时间
    cairo_show_text(cr5, worktime);
    cairo_select_font_face(cr6, "TerminusTTF", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD);
    cairo_set_font_size(cr6, 14.0);
    cairo_set_source_rgb(cr6, 1.0, 1.0, 1.0); 
...
gint show_live_setting()
{
    ...
    gchar *item2 = NULL;
    gchar *item3 = NULL;
    g_object_get(G_OBJECT (entry), "livelist", &list, "select", &select, NULL);
    if (select < -1 || select > 4)
    {
        g_print("live list property has 5 items, select wrong, (-2 < select < 5)!\n");
        return -1;
    }
    cairo_t *cr1 = cairo_create(surface);     //工作模式，图片
    cairo_t *cr2 = cairo_create(surface);     //三角箭头，图片
...
gint show_sys_setting()
{
    ...
    gchar *item2 = NULL;
    gchar *item3 = NULL;
    g_object_get(G_OBJECT (entry), "systemlist", &list, "select", &select, NULL);
    if (select < -1 || select > 3)
    {
        g_print("setting_list property has 4 items, select wrong, (-2 < select < 4)!\n");
        return -1;
    }
    cairo_t *cr1 = cairo_create(surface);     //工作模式，图片
    cairo_t *cr2 = cairo_create(surface);     //三角箭头，图片
...
gchar *get_network_image_url()
{
    ...
    g_object_get(G_OBJECT (entry), "network", &net_value, NULL);
