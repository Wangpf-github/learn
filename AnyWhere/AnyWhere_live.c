...
gint show_live()
{
    ...
    gtk_window_set_title(GTK_WINDOW(window), "live mode");
    ...
    //创建拍照模式图片窗口并加入vbox
    image_camera = gtk_image_new_from_file(get_work_mode_image_big_url(WORKMODE_LIVE));   //加载图片的路径
...
gint show_live_state()
{
    ...
    image_camera = gtk_image_new_from_file(get_state_mode_image_url(WORKMODE_LIVE));
    gtk_box_pack_start(GTK_BOX(hbox2), image_camera);
    label_time = gtk_label_new("time");
    gtk_widget_set_size_request(label_time, 100， 20);
    sprintf(work_time, "<span foreground='black' font_desc='18'> %s </span>", get_work_time_string(WORKMODE_LIVE));
